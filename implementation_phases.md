...
- **Implementation evidence:** Added framework-neutral typed API contracts in `src/api/types.py`, cookie/bearer authentication in `src/api/auth.py`, SSE event serialization in `src/api/sse.py`, multipart validation in `src/api/multipart.py`, and the versioned `ApiApplication`/`RunnerProtocol` boundary in `src/api/application.py`. Added `src/api/fastapi_app.py` as a native FastAPI facade with explicit `/api/v1` resource, capability, run, upload, and SSE routes plus a hidden standardized-error fallback; it preserves the typed core and does not import LocalRunner or HexelRunner. Added root `main.py` as the composition/deployment facade exporting `app`, `create_app`, and `build_api_application`; added pinned Uvicorn through `uv`; retained thin Catalyst delegates under `functions/api/`; documented the contract in `docs/api-reference.md`.
- **Validation evidence:** Focused P10 API suite ran **9 tests with `OK`**, including explicit OpenAPI path assertions, unique operation-ID regression coverage, and the root `main.py` facade; full repository discovery ran **112 tests with `OK`**; `uv run python -m compileall -q src functions data tests main.py` passed; `uv run uvicorn --version` reported Uvicorn 0.35.0; a local `main:app` Uvicorn smoke test returned `/docs` HTTP 200 and unauthenticated API HTTP 401; `git diff --check` passed; `.LOCK/TODO.md` and `session-ses_0754.md` remained ignored and untracked.
...
- **Known blockers:** Live Catalyst API Gateway/AppSail mounting, Catalyst Auth deployment, live CORS validation, and production SSE transport remain deployment validation items. No live service, capacity, latency, or performance claim is made.
